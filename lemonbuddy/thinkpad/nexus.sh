...
id=$(dbus-send --session --dest=org.kde.kdeconnect --print-reply /modules/kdeconnect org.kde.kdeconnect.daemon.deviceIdByName string:"Bartek-Nexus" | tail -n 1 | awk '{print $2}' | sed -e 's/^"//' -e 's/"$//')
